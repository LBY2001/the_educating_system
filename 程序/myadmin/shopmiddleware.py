...
class ShopMiddleware:
    ...
    def __call__(self, request):
        ...
        urllist = ['/teacher/login','/teacher/logout','/teacher/dologin','/teacher/verify']
        #判断当前请求url地址是否是以/mobile开头,并且不在urllist中，才做是否登录判断
        if re.match(r'^/teacher',path) and (path not in urllist):
            #判断是否登录(在于session中没有mobileuser)
            if 'teacheruser' not in request.session:
                #重定向到登录页
