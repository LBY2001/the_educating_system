...
def showindex(request):
    return render(request, "teacher/headteacher.html")
...
def dologin(request):
    ...
    try:
        ...
        if request.POST['code'] != request.session['verifycode']:
            return redirect(reverse('teacher_login') + "?errinfo=2")

        # 根据登录账号获取登录者信息
        user = User.objects.get(username=request.POST['username'])
        usertemp = request.POST['username']
        # 判断当前用户是否正常或管理员
        if user.status == 6 or user.status == 3:
            # 判断登录密码是否相同
            import hashlib
            md5 = hashlib.md5()
            ...
            if user.password_hash == md5.hexdigest():  # 获取md5值
                ...
                request.session['teacheruser'] = user.toDict()
                # 当期同学的成绩信息
                # scoreop=Scoretable.objects.get(student_id=request.POST['username'])
                # request.session['scoreinfo']=scoreop.toDict()
                ...
                # 重定向到首页
                return redirect(reverse("teacher_index"), usertemp)
            else:
                return redirect(reverse('teacher_login') + "?errinfo=5")
        else:
            return redirect(reverse('teacher_login') + "?errinfo=4")
    except Exception as err:
        print(err)
        return redirect(reverse('teacher_login') + "?errinfo=3")


def logout(request):
    ''' 执行前台退出操作 '''
    del request.session['teacheruser']
